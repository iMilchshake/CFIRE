...
Conventions:
  - Before/after columns are named:
      <metric>_before, <metric>_after_safe, <metric>_after_best
  - Delta columns (after - before) are named:
      delta_<metric>_safe, delta_<metric>_best
"""
import argparse
import sys
...
def build_output(merged: pd.DataFrame, metrics: List[str], keys: List[str]) -> pd.DataFrame:
    ...
    for m in metrics:
        ...
        out[s] = merged[s]
        out[bt] = merged[bt]
        # New delta naming scheme: delta_<metric>_safe/best
        out[f"delta_{m}_safe"] = merged[s] - merged[b]
        out[f"delta_{m}_best"] = merged[bt] - merged[b]
        out_cols.extend([b, s, bt, f"delta_{m}_safe", f"delta_{m}_best"])
    return out[out_cols]
...
def main() -> int:
    ...
    if not datasets:
        print(f"[WARN] No dataset subfolders in {root}")
        return 1
